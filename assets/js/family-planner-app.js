(() => {
  ...
  function render() {
    ...
    const fixedChart = input.program === 'Miles & More' && rate.seasonFactorMode === 'miles-only' && result.rows.every(s => s.needed === realistic.needed);
    ...
    const scenarios = fixedChart
      ? `<div class="scenario-heading"><h3>Ein Chartwert statt drei Schätzungen</h3></div>${card({ ...realistic, title:'Partnerchart als Planungsanker', detail:'gleicher Meilenwert für alle drei Annahmen' })}<p class="chart-note">Für diese Strecke gilt im Partnerchart ein fester Meilenwert. Bei dynamisch bepreisten Lufthansa-Group-Flügen kann der tatsächliche Preis abweichen. Prüfe ihn vor dem Transfer in der Buchung.</p>`
    ...
    $('resultContent').innerHTML = `<div class="focus-result"><span class="overline">${fixedChart ? 'CHART-ANKER FÜR DIE FAMILIE' : 'REALISTISCHER FAMILIENBEDARF'}</span><strong>${fmt(realistic.needed)} ${input.program === 'Miles & More' ? 'Meilen' : input.program === 'Avios' ? 'Avios' : 'Meilen'}</strong><p>${fmt(result.persons)} Personen · Hin- und Rückflug · ${leadText}</p></div>
