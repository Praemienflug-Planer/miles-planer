(() => {
  ...
  function getGithubLists() {
    return window.MILES_PLANNER_LISTS || null;
  }

  function getGithubPrograms() {
    return window.MILES_PLANNER_PROGRAMS?.programs || null;
  }

  function getGithubRates() {
    return window.MILES_PLANNER_AWARD_RATES?.rates || [];
  }
  ...
  async function ladeDropdownsAusGithub() {
    ...
    try {
      ...
      console.log("Dropdowns aus GitHub-Stammdaten geladen:", {
        ...
        programsDataStand: window.MILES_PLANNER_PROGRAMS?.dataStand
      });
    } catch (error) {
  ...
  function findGithubRate(payload) {
    return getGithubRates().find((rate) =>
      normalize(rate.ziel) === normalize(payload.ziel) &&
      normalize(rate.programm) === normalize(payload.programm) &&
      normalize(rate.klasse) === normalize(payload.reiseklasse)
    );
  }
  ...
  async function berechneMilesPlanerMitGithubFallback() {
    ...
    if (!rate) {
      ...
      if (previousBerechneMilesPlaner) {
        return previousBerechneMilesPlaner();
      }
    }
    ...
    try {
      ...
      if (previousBerechneMilesPlaner) {
        console.error("GitHub-Berechnung fehlgeschlagen. Nutze Google-Sheets-Fallback:", error);
        return previousBerechneMilesPlaner();
      }
